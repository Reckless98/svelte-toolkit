...
<div class="space-y-8">
  ...
  <ComponentPreview title="Modal Variants">
    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
      {#each variants as variant}
        <Button
          variant="outlined"
          on:click={() => openModal(variant.value)}
        >
          {variant.name} Modal
        </Button>
        ...
        >
          <div class="p-4">
            <p class="text-gray-600 dark:text-gray-400 mb-4">
              This is a {variant.name.toLowerCase()} modal example. You can customize the content, 
              styling, and behavior according to your needs.
            </p>
            <div class="flex justify-end space-x-2">
              <Button variant="ghost" on:click={() => closeModal(variant.value)}>
                Cancel
              </Button>
              <Button on:click={() => closeModal(variant.value)}>
                Confirm
              </Button>
            </div>
          </div>
  ...
  <ComponentPreview title="Modal Animations">
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      {#each animations as animation}
        <Button
          variant="outlined"
          on:click={() => openModal(animation.value + 'Animation')}
        >
        ...
        >
          <div class="p-4">
            ...
            <div class="flex justify-end">
              <Button on:click={() => closeModal(animation.value + 'Animation')}>
                Close
              </Button>
            </div>
          </div>
  ...
  <ComponentPreview title="Modal Positions">
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
      {#each positions as position}
        <Button
          variant="outlined"
          on:click={() => openModal(position.value + 'Position')}
        >
        ...
        >
          <div class="p-4">
            ...
            <div class="flex justify-end">
              <Button on:click={() => closeModal(position.value + 'Position')}>
                Close
              </Button>
            </div>
          </div>
  ...
  <ComponentPreview title="Practical Examples">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      <!-- Form Modal -->
      <Button variant="accent" on:click={() => openModal('withForm')}>
        Contact Form
      </Button>
      ...
      <!-- Gallery Modal -->
      <Button variant="secondary" on:click={() => openModal('gallery')}>
        Image Gallery
      </Button>
      
      <!-- Settings Modal -->
      <Button variant="ghost" on:click={() => openModal('settings')}>
        Settings Panel
      </Button>
    </div>
    ...
    >
      <form on:submit|preventDefault={handleSubmit} class="p-4 space-y-4">
        ...
        <div class="flex justify-end space-x-2 pt-4">
          <Button variant="ghost" on:click={() => closeModal('withForm')}>
            Cancel
          </Button>
          <Button type="submit">
            Send Message
    ...
    >
      <div class="p-4 text-center">
        ...
        <div class="flex justify-center space-x-2">
          <Button variant="ghost" on:click={() => closeModal('confirmation')}>
            Cancel
          </Button>
      ...
      <div class="p-4">
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {#each Array(12) as _, i}
            <div class="aspect-square bg-gradient-to-br from-gray-200 to-gray-300 rounded-lg flex items-center justify-center">
    ...
    >
      <div class="p-4 space-y-6">
        <div>
          ...
          <div class="space-y-4">
            <div class="flex items-center justify-between">
              ...
              <button class="relative inline-flex h-6 w-11 items-center rounded-full bg-gray-200 transition-colors focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2">
                <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform translate-x-1"></span>
              </button>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-sm text-gray-700 dark:text-gray-300">Notifications</span>
              <button class="relative inline-flex h-6 w-11 items-center rounded-full bg-accent transition-colors focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2">
        ...
        <div class="flex justify-end space-x-2 pt-4 border-t">
          <Button variant="ghost" on:click={() => closeModal('settings')}>
            Cancel
          </Button>
          <Button on:click={() => closeModal('settings')}>
            Save Changes
          </Button>
        </div>
      </div>
