...
<script lang="ts">
			...
			if (!ticking) {
				requestAnimationFrame(() => {
					...
					if (background && container) {
						...
						isInView =
							containerTop <= scrollY + windowHeight && containerTop + containerHeight >= scrollY;
	...
	$: animationStyles = (() => {
		if (!contentVisible)
			return { opacity: 0, transform: getTransformForType(animationType, false) };
		return { opacity: 1, transform: getTransformForType(animationType, true) };
	})();

	function getTransformForType(type: string, visible: boolean): string {
...
{#if mounted}
    ...
    "
		...
		{#if isInView && fullHeight}
			<div
				class="absolute bottom-8 left-1/2 transform -translate-x-1/2 text-white/60 animate-bounce"
			>
