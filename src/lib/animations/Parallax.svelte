...
<script lang="ts">
	...
	// Props
	export let speed: number = 0.5; // Parallax speed (0-1)
	export let direction: 'up' | 'down' | 'left' | 'right' = 'up';
	export let overflow: 'hidden' | 'visible' = 'hidden';
	export let containerClass: string = ''; // Additional class for container
	...
	onMount(() => {
		...
		const handleScroll = () => {
			...
			if (container) {
				...
				if (offsetTop + elementHeight >= scrollY && offsetTop <= scrollY + windowHeight) {
...
>
	<div bind:this={content} class="will-change-transform" style={transformStyle}>
		<slot />
	</div>
</div>
