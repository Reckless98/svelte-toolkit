...
<script lang="ts">
	...
	type InputAnimation = 'none' | 'pulse' | 'glow' | 'float';

	// Props with TypeScript types
	export let type: InputType = 'text';
	export let variant: InputVariant = 'standard';
	...
	$: variantClasses = {
		...
		underlined:
			'bg-transparent border-0 border-b-2 border-gray-300 dark:border-gray-700 rounded-none px-0 focus:border-blue-500',
...
<div class="{width} relative">
	...
	<div class="relative">
		...
		{#if label && variant === 'floating'}
			<label
				for={id}
				class="absolute transition-all duration-200 pointer-events-none {labelFloating
					? '-top-2 left-2 text-xs bg-white dark:bg-gray-800 px-1'
