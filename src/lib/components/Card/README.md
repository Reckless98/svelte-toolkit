...
```svelte
<Card clickable={true} title="Clickable Card" on:click={handleCardClick}>
	<p>This card is clickable</p>
</Card>

<!-- As a link -->
<Card href="/article/123" title="Article Card">
...
```svelte
<Card variant="glass" hover="glow" class="text-center">
	<svelte:fragment slot="header">
	...
	<svelte:fragment slot="footer">
		<a href={feature.learnMoreUrl} class="text-blue-600 hover:underline"> Learn More → </a>
	</svelte:fragment>
</Card>
```
...
<div class="columns-1 md:columns-2 lg:columns-3 gap-6">
	{#each items as item}
		<Card title={item.title} class="break-inside-avoid mb-6">
			<p>{item.content}</p>
		</Card>
	{/each}
</div>
...
<Card class="w-full sm:w-80 md:w-96" title="Responsive Card">
