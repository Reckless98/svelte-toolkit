...
<script lang="ts">
  ...
  type NavigationVariant = 'horizontal' | 'vertical' | 'dropdown' | 'breadcrumb' | 'tabs' | 'modern' | 'floating';
...
<nav 
  class="relative {sticky ? 'sticky top-0 z-40' : ''} {transparent ? 'bg-transparent' : variant === 'modern' || variant === 'floating' ? '' : themeClasses[theme]} {variant === 'horizontal' ? 'border-b' : ''} {shadowClasses[shadow]} {animationClasses[animation]} transition-all duration-300 {variant === 'floating' ? '' : 'w-full'}"
...
>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex items-center {alignmentClasses[alignment]} {sizeClasses[size]}">
      ...
      <!-- Desktop Navigation -->
      {#if variant === 'horizontal'}        <div class="hidden md:block {logo ? 'ml-10' : ''}">
            ...
            {#each items as item}
              {#if item.children && item.children.length > 0}
                ...
                <div class="relative">
                  ...
                  >
                    {item.label}
                    <svg class="ml-1 h-4 w-4 transform {dropdownOpen[item.id] ? 'rotate-180' : ''} transition-transform duration-200" fill="currentColor" viewBox="0 0 20 20">
              ...
              {:else}
                <!-- Regular item -->
                <button
                  class="px-3 py-2 rounded-md text-sm font-medium hover:bg-opacity-75 transition-colors duration-200 {activeItem === item.id ? 'bg-opacity-50' : ''}"
    ...
    {#if mobileMenuOpen && variant === 'horizontal'}
      <div class="md:hidden" transition:fly={{ y: -20, duration: 300 }}>
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">          {#each items as item}
            {#if item.children && item.children.length > 0}
              <div>
                <button
            ...
            {:else}
              <button
                class="block px-3 py-2 rounded-md text-base font-medium hover:bg-opacity-75 transition-colors duration-200 w-full text-left {activeItem === item.id ? 'bg-opacity-50' : ''}"
                on:click={() => handleItemClick(item)}
              >
                {item.label}
              </button>
      ...
      {#each items as item}
        {#if item.children && item.children.length > 0}
          <div>
            ...
            >
              {item.label}
              <svg class="h-4 w-4 transform {dropdownOpen[item.id] ? 'rotate-180' : ''} transition-transform duration-200" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
              </svg>
            </button>
            {#if dropdownOpen[item.id]}
        ...
        {:else}
          <button
            class="px-3 py-2 rounded-md text-sm font-medium hover:bg-opacity-75 transition-colors duration-200 w-full text-left {activeItem === item.id ? 'bg-opacity-50' : ''}"
            on:click={() => handleItemClick(item)}
          >
            {item.label}
          </button>
  ...
  {#if variant === 'breadcrumb'}
    <div class="flex items-center space-x-2 px-4 py-2">
      {#each items as item, index}
        ...
        {/if}
        <button
          class="text-sm {index === items.length - 1 ? 'font-medium text-gray-900' : 'text-gray-500 hover:text-gray-700'} transition-colors duration-200"
          on:click={() => handleItemClick(item)}
        >
          {item.label}
        </button>
  ...
  {#if variant === 'tabs'}
    <div class="border-b border-gray-200">
      <div class="flex space-x-8 px-4">
        {#each items as item}
          <button
            class="py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200 {activeItem === item.id ? 'border-accent text-accent' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
            on:click={() => handleItemClick(item)}
          >
            {item.label}
          </button>
  ...
  <!-- Modern Navigation -->
  {#if variant === 'modern'}
    <div class="relative bg-white/10 backdrop-blur-md border border-white/20 rounded-xl mx-4 my-2 shadow-lg">
      <div class="flex items-center justify-between px-6 py-4">
        ...
        <div class="hidden md:flex items-center space-x-1">
          {#each items as item}
            <button
              class="relative px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 hover:bg-white/20 hover:scale-105 transform-gpu {activeItem === item.id ? 'bg-white/30 text-blue-600' : 'text-gray-700 hover:text-gray-900'}"
              on:click={() => handleItemClick(item)}
            >
              {item.label}
              {#if activeItem === item.id}
                <div class="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-blue-600 rounded-full animate-pulse"></div>
              {/if}
            </button>
          {/each}
      ...
      {#if mobileMenuOpen}
        <div class="md:hidden border-t border-white/20" transition:fly={{ y: -20, duration: 300 }}>
          <div class="px-4 py-3 space-y-2">
            {#each items as item}
              <button
                class="block w-full px-4 py-3 rounded-lg text-left text-sm font-medium transition-all duration-200 hover:bg-white/20 hover:scale-[1.02] transform-gpu {activeItem === item.id ? 'bg-white/30 text-blue-600' : 'text-gray-700'}"
                on:click={() => handleItemClick(item)}
              >
                {item.label}
              </button>
  ...
  {#if variant === 'floating'}
    <div class="fixed top-4 left-1/2 transform -translate-x-1/2 z-50">
      <div class="bg-white/80 backdrop-blur-xl border border-gray-200/50 rounded-full px-6 py-3 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 transform-gpu">
        <div class="flex items-center space-x-6">
          ...
          <div class="flex items-center space-x-1">
            {#each items as item}
              <button
                class="relative px-3 py-2 rounded-full text-sm font-medium transition-all duration-300 hover:bg-gray-100 hover:scale-110 transform-gpu {activeItem === item.id ? 'bg-blue-100 text-blue-600' : 'text-gray-700 hover:text-gray-900'}"
                on:click={() => handleItemClick(item)}
              >
                {item.label}
                {#if activeItem === item.id}
                  <div class="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-1 h-1 bg-blue-600 rounded-full animate-bounce"></div>
                {/if}
              </button>
            {/each}
          </div>
